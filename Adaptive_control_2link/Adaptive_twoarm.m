...
% figure('Name', 'I/p- error for joint in Adaptive control')
% plot(T, error(2,1:size(T,1)), 'b-');
% legend('joint 2 error')
% title('Error for joint in Adaptive Control');
%
% 
...
% xlabel('time')
% ylabel('Mass')
% legend('Estimated Mass', 'Actual Mass: sint + 3')
% title('Mass error for Adaptive Control');

curve = animatedline;
AB = X(:,5);
CD = 2*ones(size(T,1),1);
for i = 1:20
    addpoints(curve, T(i),AB(i));
    drawnow limitrate
end
